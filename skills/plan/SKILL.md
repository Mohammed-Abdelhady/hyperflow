...
Load [approval-gates.md](references/approval-gates.md) before entering the first plan gate. It contains the gate table, the build-location invariants, the structured-input fallback, and the exact Hyperflow Question behavior. The build-location gate remains mandatory on every run; this reference only keeps the detailed gate contract out of the always-loaded startup surface.
...
**Decision cards (architecture / privacy / storage / test shape):** when a clarify item is a multi-option irreversible choice (framework, DB shape, auth vendor, test runner, public API surface), present it as a [decision card](../../docs/decision-cards.md) using [`templates/decision-card.md`](../../templates/decision-card.md) — numbered options, one recommended, max 4 choices. After the user locks a number, **same turn** run the deterministic writer `python3 <plugin-root>/scripts/decision-card.py lock --memory-dir .hyperflow/memory --title "..." --choice "..." --why "..." --revisit-if "..."` (add `--chooser` when known). It validates fields, adds the `[decision]` tag, and refuses duplicate titles; do not hand-edit or silently overwrite an existing lock. Do not re-ask locked decisions later unless requirements conflict. `python3 <plugin-root>/scripts/decision-card.py validate --memory-dir .hyperflow/memory` checks legacy memory before relying on it.
