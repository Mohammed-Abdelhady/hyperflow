...
| **Test cases** | Concrete input → expected-output table (min 3: happy path + edge case + error/empty case). Worker implements against the table AND, for code tasks, writes the verifying test code as part of the deliverable. Per-batch Reviewer runs / verifies each case to confirm PASS. Format: \\| # \\| Name \\| Input \\| Expected \\| Notes \\|. Omit ONLY when genuinely test-impossible (one-line README typo, etc.); in that case state `Test cases: N/A — <why>` so the omission is deliberate. |
