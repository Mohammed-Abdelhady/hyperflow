...
**Provider-neutral floor.** Skills express intent with canonical ops from [runtime-contract.md](runtime-contract.md) (`spawn`, `wait`, `structured_question`, `skill_continuation`, `edit`, `shell`, `web_research`, `background`, `usage_metrics`). Host adapters map those ops onto live tools. Cross-skill handoffs, argument propagation, and structural gates follow [chain-router.md](chain-router.md). Missing structured UI → Hyperflow Question chat block + end turn. Missing subagents → labelled inline worker, then separate labelled inline reviewer. Missing native skill handoff → load the target `skills/<name>/SKILL.md` completely and continue inline. **Never** route to retired targets `spec` or `scope` as skills — plan owns sharpen/design/decompose; those words may still appear as artefact paths (`.hyperflow/specs/`) or ordinary English.
...
1. **Zero confirmations.** No "should I?", "shall I proceed?". Execute. (But clarification questions via the `structured_question` op are REQUIRED — see rule 8.)
...
7. **Auto-accept all permissions.** File, terminal, tool — never pause. Security blocklist and irreversibility gates still apply ([security.md](security.md)).
8. **Clarification is mandatory, confirmation is banned. Structural gates ALWAYS fire. Invented gates NEVER fire.**
   ...
   - **REQUIRED:** `structured_question` (native structured UI when present; otherwise Hyperflow Question chat block — [runtime-contract.md](runtime-contract.md)) for understanding WHAT to build, WHERE ambiguity exists, WHICH approach to take. These happen at:
   ...
   - **Structural gates** — plan build-location, dispatch operational + end-of-chain audit/deploy/PR, design section/handoff, phase-dispatch scope, **audit fix gate**, deploy push/commit-inclusion, PR posting/merge, and `SECURITY_VIOLATION` halt — fire whenever their documented precondition is met ([chain-router.md](chain-router.md)). Clarification questions are not structural gates: ask them only for unresolved material choices after inspection.
   - **The session / build-location gate controls WHERE the chain runs, not whether material ambiguity exists.** Clear work can ask zero clarification questions; irreversible ambiguity still fires its dedicated escalation gate.
   - **Portable / single-agent fallback (Codex, OpenCode, Grok, headless with a chat channel):** if the host does not expose structured question UI, the structural gate still fires in chat. Print a compact `Hyperflow Question` block with the same question, numbered options, and `(Recommended)` marker where the doctrine requires one (binary action gates carry **no** marker), then **end the turn** and wait for the user's reply. Never silently pick the recommendation, never downgrade the gate to a status update, and never treat the lack of popup UI as permission to skip required questions. Headless with **no** interactive channel at a structural gate → error and stop.
   - **File-first artefacts: long-form work product lives in files under `.hyperflow/`, never inline in chat and never scattered into other repo locations. Format per [`artefact-format.md`](artefact-format.md) (classic) or [`artefact-data.md`](artefact-data.md) (viewer mode — the default: emit compact JSON via `scripts/artefact.py` + a slim stub, rendered by the local `hyperflow view`).** Every planning artefact the orchestrator produces — feature specs, design sections, task decompositions, audit findings, audit-fix specs, decision logs — MUST live under one of three canonical homes:
   ...
   - **Clarification fires AFTER analysis, never before.** Every clarification stage runs *after* the orchestrator has (a) read the relevant code via Searcher dispatches, (b) loaded project context from `.hyperflow/profile.md` / `architecture.md` / `conventions.md`, and (c) produced an analysis brief (plan Analyst, plan/decompose Searcher coverage, audit context-gathering Searcher). Asking the user *before* the system has done its homework wastes the user's time on questions the codebase already answers — *"where should the new auth guard live?"* is a clarification question only if Searcher already mapped the auth surface and found two equally plausible targets. If it found one, the orchestrator picks it. If it found zero, the orchestrator escalates the search, not the user. Concrete ordering for **`/hyperflow:plan`** (the live chain-starter — retired `spec`/`scope` skills are **not** targets): Triage (Step 1) → Context Searchers (Step 3, concurrent with triage) → multi-dim Analyst (Step 4 when needed) → THEN Clarify (Step 5); on the bounce/decompose path, research Searchers still run before any material questions. **`/hyperflow:audit`** gathers context (Step 2) before any fix-gate question (Step 6). Every clarification question MUST be tied to a specific research finding — *"Searcher mapped both A and B, which is intended?"* — never a blank-slate *"where should this go?"* that the orchestrator never tried to answer first.
   - **Invented gates are BANNED.** The orchestrator MAY NOT fire `structured_question` for anything outside the structural-gates list above. Specifically banned patterns:
   ...
   - **Never add a "Type something" / "Other" option manually.** Native structured-question UIs already include freeform affordances. Adding it as option 3 (or 4) is dead UI and pads the choice list. Chat-gate blocks list only the real options.
...
Dispatch every agent via the **`spawn`** op (or a labelled inline phase when `spawn` is unavailable — [runtime-contract.md](runtime-contract.md)). **No per-role model selection** — every child inherits the current session model. Roles differ by *brief and responsibility*, not by model:
...
If a worker returns `ESCALATE: <reason>`, the orchestrator upgrades the flow profile per [escalation.md](escalation.md) rules. If risk becomes irreversible mid-flight, the orchestrator HALTS and fires `structured_question` for explicit consent (Hyperflow Question chat block when structured UI is missing). See [escalation.md](escalation.md) for paths and token accounting.
...
2. **Parallel by default.** Sub-tasks that don't share state get dispatched simultaneously in a single turn with multiple sibling `spawn`s (or sequential labelled inline phases when `spawn` is unavailable — never claim parallel when work was serial).
...
9. **Agent labels.** Before every `spawn` (or labelled inline phase), print a single elegant line. No icons, no brackets, no emoji. Format: `Role — short description` (em-dash separator, description lowercase, under 80 chars).
...
10. **Usage tracking.** Track every agent dispatch via the **`usage_metrics`** op ([runtime-contract.md](runtime-contract.md)). Prefer host-reported input/output/cache tokens and observed wall-clock / cumulative durations from the usage ledger — **never** parse a single host's completion UI chrome as a universal source, and **never fabricate** tokens, durations, or parallelism ratios. When metrics are missing, print `unavailable` (or mark explicit estimates with `estimated=true`). Wall-clock vs cumulative (when both observed) proves whether `parallel:N` dispatches actually ran concurrent. After the task completes, print a usage summary. Triage, plan depth, and profile lines surface up-front when a flow profile is in play. See [escalation.md](escalation.md) for the canonical format and [output-style.md](output-style.md) for visual rules.
    ...
    **What counts as a decision agent:**
    - Every batch review MUST be a dispatched `spawn` (or a **separately labelled** inline reviewer phase when `spawn` is unavailable) — reading files yourself mid-coordination and saying "looks good" is NOT a review and does NOT count as a Reviewer.
...
12.1. **Trivial steps may be performed inline by the orchestrator without a `spawn` wrapper.** A step qualifies as trivial AND inline-allowed IF AND ONLY IF all of:
   ...
   If the orchestrator discovers mid-step that the work requires generation or research, it MUST abort the inline path and dispatch via `spawn` (or a labelled inline worker phase). Trivial-eligibility is evaluated at step-start, not assumed throughout.
...
12.2. **Sub-phase decomposition.** Every non-trivial Step (per §12, not §12.1-exempt) MUST decompose into ≥ 2 named sub-phases unless intrinsically atomic. A **sub-phase** is a named unit `Step Na`, `Step Nb`, `Step Nc`, ... inside a parent Step.
   ...
   **Sub-phase rules:**
   ...
   8. **Atomic exemption.** A Step is atomic when its entire body is one of: a single `structured_question` gate · a single mechanical decision (file existence, git status, route choice) · a single Worker → Reviewer pair with no independent angles to fan out across. In all other cases, decompose into ≥ 2 sub-phases.
...
13. **Latency discipline.** Reduce wall-clock time by restructuring *when* and *how* dispatches fire — never by cutting who reviews what or which role does it.
   - **P1 — Parallelize sibling workers.** Sub-tasks that share a common upstream input and have no inter-dependency MUST be dispatched in a single turn with parallel `spawn`s. Never sequentialize siblings when the host can run them concurrently.
...
Background agents are an opt-in extension of Layer 3 dispatch. They use the host **`background`** op when present ([runtime-contract.md](runtime-contract.md)); the chain does not wait, results are integrated later. When `background` is **unavailable**, stay **foreground-only** — no fake notifications. Three legitimate patterns when the op exists: **latency reduction** (Layer 5 gates fired while next batch runs), **observers** (CI watcher after push), **speculative prefetch** (refresh `.hyperflow/<analysis>.md` while user picks the next skill). Full doctrine: [`background-agents.md`](background-agents.md). Management surface: `/hyperflow:background list|show|cancel|prune`. Hard rules (apply alongside rule 8): no `structured_question` from background, no independent commits, no background Reviewers (reviewers gate decisions), no background-of-background, mandatory cancellation on chain abort, max 30-min runtime cap, OFF by default per skill.
...
3. **Memory optimize** — always-on: rebuilds `memory/index.md` (+ `.checksums`) and flags oversized durable files for compact. An **auto-reap never removes a durable memory entry**; entry-dropping is opt-in via `cleanup.dropOrphanRefs` (default `false`) and **quarantines** orphaned entries to `memory/archive/YYYY-MM.md` (never hard-deleted). See [memory-system.md](memory-system.md).
...
All skills inherit this doctrine — they reuse the same worker/reviewer prompts, **session-model role separation** (never model-tier routing), security policies, memory system, [runtime-contract.md](runtime-contract.md) ops, and [chain-router.md](chain-router.md) transitions. Each skill file is short (~80–150 lines) and references shared files in `skills/hyperflow/*.md`. Live continuation targets only (`plan`, `dispatch`, `audit`, `deploy`, …) — **never** retired `spec` / `scope` skill names.

Hand-off pattern (via `skill_continuation` — native Skill when present, otherwise full target `SKILL.md` load + inline continuation):
- `/hyperflow:plan` → sharpens, designs, and decomposes (max thinking) → produces a task file → **stops** at the build-location gate (always asked): build here → continues into `/hyperflow:dispatch`; another session → writes a handoff package; stop → keeps the plan. Plan never auto-implements.
- `/hyperflow:dispatch` → runs batches + final review → end-of-chain gates for `/hyperflow:audit` or `/hyperflow:deploy` (no auto-push)
- `/hyperflow:audit` → multi-level review → **fix gate** (structural) → on fix choice continues into `/hyperflow:plan` with a scoped audit-fix spec (plan still owns its own build-location gate — no blind patch)
...
- Label a batch `parallel:N` but dispatch the calls across separate messages — that's serial, not parallel. The wall-clock / cumulative ratio will land ≥ 0.8 and expose it. Investigate and re-dispatch with all N sibling `spawn`s in a single turn (or mark the work sequenced inline honestly).
- Fire a `structured_question` between batches in `auto` mode — "transparency checkpoint", "midway sanity check", "scope re-confirmation", "cost heads-up", or any rephrasing of *"should I keep going?"*. Per rule 8, auto means finish the chain. The only gates between batches are the structural ones (`SECURITY_VIOLATION` halt, escalation crossing the irreversibility boundary, inter-batch advance in *manual* mode). Status prints are fine; status *questions* are banned.
- Fabricate usage tokens / parallelism ratios when `usage_metrics` is unavailable, or route to retired skills `spec` / `scope` as live chain targets.
- Skip the **audit fix gate** after NEEDS_FIX findings, or silently auto-fix without a plan decomposition path ([chain-router.md](chain-router.md)).
