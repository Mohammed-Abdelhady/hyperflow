...
8. **Clarification is mandatory, confirmation is banned. Structural gates ALWAYS fire. Invented gates NEVER fire.**
   ...
   - **File-first artefacts: long-form work product lives in files, never inline in chat.** Specs, design sections, audit findings, task plans, and decomposition artefacts MUST be written to files under `.hyperflow/specs/`, `.hyperflow/tasks/`, or `.hyperflow/audits/` — and the approval/fix gate references the file path, not the content. Inline content in chat is ephemeral (unscrollable, uneditable, lost across context compactions); a file is reviewable in the user's editor, diffable, persistent. **Concrete enforcement:**
