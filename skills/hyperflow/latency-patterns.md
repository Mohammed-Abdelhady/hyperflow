...
**What it is:** Run deterministic `route-task.py --auto-observe` first. It may extract an explicit relative file reference and prove only a narrow safe edit (typo/spelling/whitespace/format, rename, reword, docs/comment/test update, or missing import). Dispatch the lean structured-output Classifier only when that proof fails.
