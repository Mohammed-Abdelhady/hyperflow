...
3. As each worker returns:
   ...
   - Immediately dispatch a thinking-tier reviewer per [reviewer-prompt.md](../hyperflow/reviewer-prompt.md). Print `**Reviewer** — reviewing <subtask>`.
...
4. After the full batch — synthesize learnings, check off the batch in the task file, run quality gates per [quality-gates.md](../hyperflow/quality-gates.md).

### Step 3 — Final Integration Review

Mandatory and **separate from batch reviews**. Dispatch a thinking-tier reviewer with the full set of changed files. Print `**Reviewer** — final integration review`. Verdict required — `PASS` / `NEEDS_FIX` / `SECURITY_VIOLATION`.
