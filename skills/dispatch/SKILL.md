...
### Usage ledger + budget boundaries

The normal-flow usage contract is a lazy reference. Before the first worker, Composer, Reviewer, or gate dispatch, load [usage-ledger.md](references/usage-ledger.md) and apply its recording and budget-boundary rules. The contract remains mandatory for every normal-flow agent call; this indirection only keeps the dispatch startup prompt smaller. Inline-fast remains exempt because it launches zero agents and records foreground evidence instead.
