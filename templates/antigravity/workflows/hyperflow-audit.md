...
4. Fix-gate (AskUserQuestion, only if Critical/Important): `Fix all (Recommended) / Critical+Important / Critical only / No`. On a fix choice, route via `/hyperflow-plan` → `/hyperflow-dispatch`. `SECURITY_VIOLATION` → surface immediately, no gate.
